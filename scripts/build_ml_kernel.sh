...
cp -a ../specs-el$EL_VERSION/kernel-ml-aufs-$VERSION.spec build/kernel-ml-aufs.spec
...
mock -r $MOCK_CONFIG --buildsrpm --spec kernel-ml-aufs.spec --sources . --resultdir rpms > logs/srpm_generation.log 2>&1
...
if [ -z "$SRPM_ONLY" ]; then
  ...
  mock -r $MOCK_CONFIG --rebuild --resultdir rpms rpms/kernel-ml-aufs-$FULL_VERSION-1.$RPM_EL_VERSION.src.rpm > logs/rpm_generation.log 2>&1
  if [ $? -eq 0 ]; then
...
if [ $? -eq 0 ]; then
  ...
  copr-cli build kernel-ml-aufs --nowait -r $MOCK_CONFIG rpms/kernel-ml-aufs-$FULL_VERSION-1.$RPM_EL_VERSION.src.rpm > logs/copr_submission.log 2>&1
  if [ $? -eq 0 ]; then
