...
mock -r epel-$EL_VERSION-$ARCH --buildsrpm --spec kernel-ml-aufs-$VERSION.spec --sources . --resultdir rpms > logs/srpm_generation.log 2>&1
...
if [ -z "$SRPM_ONLY" ]; then
  ...
  mock -r epel-$EL_VERSION-$ARCH --rebuild --resultdir rpms rpms/kernel-ml-aufs-$FULL_VERSION-1.$RPM_EL_VERSION.src.rpm > logs/rpm_generation.log 2>&1
  if [ $? -eq 0 ]; then
